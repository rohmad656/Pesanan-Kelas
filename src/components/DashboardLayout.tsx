...
export default function DashboardLayout() {
  ...
  const initialRole = React.useRef<string | null>(
    localStorage.getItem("user_role_last_session"),
  );
  ...
  useEffect(() => {
    if (
      profile?.role &&
      initialRole.current &&
      profile.role !== initialRole.current
    ) {
      // Role has changed!
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === "added") {
            ...
            if (
              !data.isRead &&
              data.createdAt &&
              !toastHistory.current.has(notifId)
            ) {
              const now = Date.now();
        ...
        const newNotifs = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as any,
        );
        // Deduplicate based on ID to avoid double entries in UI
        const unique = Array.from(
          new Map(newNotifs.map((item) => [item.id, item])).values(),
        ).sort(
          (a: any, b: any) =>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#1E1E2F] text-slate-900 dark:text-[#F5F5F5] flex overflow-hidden transition-colors duration-200">
      ...
      >
        <div className="p-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-brand-700 dark:text-brand-dark-accent tracking-tight">
              {PROJECT_NAME}
            </h1>
              ...
              >
                <Icon
                  className={cn(
                    ...
                    isActive
                      ? "text-brand-600 dark:text-brand-dark-accent"
                      : "text-slate-400 dark:text-[#6A6A8A] group-hover:text-brand-600 dark:group-hover:text-brand-dark-accent",
                  )}
                />
        ...
        <div className="p-4 border-t border-slate-200 dark:border-[#3F3F5A]/30">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-slate-900 dark:text-[#F5F5F5] group-hover:text-brand-700 dark:group-hover:text-brand-dark-accent transition-colors">
                {profile?.name}
              </p>
                  ...
                  adminContacts.map((contact) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] font-bold text-slate-800 dark:text-[#F5F5F5] truncate">
                          {contact.name}
                        </p>
                        <p className="text-[9px] text-green-600 dark:text-green-400 font-medium truncate">
                          {contact.whatsapp}
                        </p>
                      </div>
                    </a>
                  ...
                  <div className="space-y-1">
                    {SUPPORT_EMAIL ? (
                      ...
                      <p className="text-[10px] text-slate-500 italic">
                        Hubungi admin kampus untuk bantuan.
                      </p>
                    )}
                  </div>
                )}
        ...
        {showLogoutConfirm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            ...
            >
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-[#F5F5F5] mb-2">
                Konfirmasi Keluar
              </h3>
              <p className="text-slate-600 dark:text-[#B4B4C8] text-sm mb-6">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen">
              ...
              <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-[#27273A] dark:shadow-2xl dark:shadow-black/40 border border-slate-200 dark:border-[#3F3F5A]/50 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 origin-top-right z-[60] text-left">
                ...
                <div className="transform translate-y-[-10px] group-hover:translate-y-0 transition-transform duration-300 ease-out">
                  <div className="p-3 border-b border-slate-200 dark:border-[#3F3F5A]/30 flex justify-between items-center">
                    <h3 className="font-semibold text-slate-900 dark:text-[#F5F5F5] text-sm">
                      Notifikasi
                    </h3>
                    {unreadCount > 0 && (
                      <span
                        onClick={markAllRead}
                        className="text-xs text-brand-600 dark:text-brand-dark-accent font-medium cursor-pointer hover:underline"
                      >
                        Tandai semua dibaca
                      </span>
                    )}
                  ...
                  <div className="p-2 max-h-80 overflow-y-auto space-y-1 custom-scrollbar">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((notif) => {
                        ...
                        let iconColorClass =
                          "text-slate-600 dark:text-[#B4B4C8]";
                        ...
                        // Category: Laporan / Issue
                          ...
                          ["issue", "laporan", "report"].includes(notif.type)
                        ) {
                          IconComponent = AlertCircle;
                          iconColorClass = "text-amber-500 dark:text-amber-400";
                          if (
                            notif.title.toLowerCase().includes("selesai") ||
                            notif.message.toLowerCase().includes("selesai")
                          ) {
                            badgeColorClass =
                              "bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20";
                            badgeText = "Selesai";
                        ...
                        // Category: Info
                          ...
                          IconComponent = Bell;
                          iconColorClass =
                            "text-brand-500 dark:text-brand-dark-accent";
                        }
                        ...
                        return (
                          ...
                          >
                            <div
                              className={cn(
                                "w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5 bg-slate-100 dark:bg-[#1E1E2F] border border-slate-200 dark:border-[#3F3F5A]/50 transition-transform group-hover/item:scale-110",
                                iconColorClass,
                              )}
                            >
                            ...
                            <div className="flex-1 min-w-0 pr-2">
                              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                                <p
                                  className={cn(
                                    "text-sm text-slate-900 dark:text-[#F5F5F5] truncate font-bold",
                                  )}
                                >
                                  {notif.title}
                                </p>
                                {badgeText && (
                                  <span
                                    className={cn(
                                      "text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider whitespace-nowrap",
                                      badgeColorClass,
                                    )}
                                  >
                                    {badgeText}
                  ...
                  <div className="p-2 border-t border-slate-200 dark:border-[#3F3F5A]/30 text-center">
                    <Link
                      to="/pesan"
                      className="text-xs text-brand-600 dark:text-brand-dark-accent font-bold hover:underline"
                    >
                      Baca Semua Pesan
                    </Link>
                  </div>
        ...
        <div className="flex-1 overflow-auto flex flex-col focus:outline-none relative">
              ...
              >
                ...
                >
                  ...
                  <p className="text-slate-600 dark:text-[#B4B4C8] mb-6">
                    Selamat! Permintaan Anda untuk menjadi{" "}
                    <span className="font-bold text-brand-600 dark:text-brand-dark-accent capitalize">
                      {hasRoleChanged}
                    </span>{" "}
                    telah disetujui oleh Administrator.
                  </p>
                  <div className="bg-brand-50 dark:bg-brand-500/5 p-4 rounded-2xl mb-6">
                    <p className="text-xs text-brand-700 dark:text-brand-dark-accent font-bold uppercase tracking-widest mb-1 font-mono">
                      Penting
                    </p>
                    <p className="text-sm text-slate-700 dark:text-[#D1D1E0]">
                      Anda akan segera dialihkan ke halaman login untuk
                      memperbarui sesi akun Anda.
                    </p>
                  </div>
              ...
              <div className="bg-brand-50/50 dark:bg-brand-dark-accent-light/5 border-b border-brand-200 dark:border-brand-dark-accent-light/20 px-4 md:px-8 py-3 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-[#B4B4C8]">
                      Lengkapi NIM dan No. WhatsApp untuk menggunakan fitur
                      pemesanan ruangan.
                    </p>
                  </div>
                </div>
          ...
          <footer className="px-4 md:px-8 py-4 border-t border-slate-200 dark:border-[#3F3F5A]/30 text-[10px] text-slate-400 dark:text-[#B4B4C8]/30 text-center mt-auto">
            © 2026 {PROJECT_NAME} • Modern Academic Experience • Made with ❤️
