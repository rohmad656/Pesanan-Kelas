...
export default function ResetPassword() {
  ...
  useEffect(() => {
    const verifyInitialCode = async () => {
      ...
      try {
        const response = await verifyResetCode(initialOobCode);
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const response = await verifyResetCode(codeToVerify);
      setEmail(response.email || "");
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#1E1E2F] flex items-center justify-center p-4">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-600 dark:text-[#B4B4C8]">
      ...
      >
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            {email && (
              <p className="text-slate-500 dark:text-[#B4B4C8] text-sm mt-1">
          ...
          {error ? (
            <div className="space-y-6 text-center">
              ...
              <button
                onClick={() => {
                  setError("");
                  manualCode ? setIsCodeValidated(false) : navigate("/login");
                }}
              ...
              >
                ...
                {manualCode ? "Coba Lagi" : "Kembali ke Login"}
              </button>
            </div>
          ) : success ? (
            <div className="text-center space-y-6">
              <div className="p-6 bg-green-50 dark:bg-green-900/20 rounded-2xl border border-green-100 dark:border-green-800/30">
                ...
                <p className="text-slate-600 dark:text-[#B4B4C8] text-sm mt-3 leading-relaxed">
                  Kata sandi Anda telah diperbarui. Sistem telah mengirimkan
                  notifikasi konfirmasi ke email Anda secara otomatis.
                </p>
                <div className="mt-4 pt-4 border-t border-green-200 dark:border-green-800/30">
                  <p className="text-xs text-slate-500 flex items-center justify-center gap-1">
                    <Loader2 className="w-3 h-3 animate-spin" /> Mengalihkan ke
                    login...
                  </p>
                </div>
              </div>
            ...
            <form onSubmit={handleVerifyCode} className="space-y-6">
              <div className="p-4 bg-slate-50 dark:bg-[#2D2D44] border border-slate-200 dark:border-[#3F3F5A]/30 rounded-xl">
                <p className="text-sm text-slate-600 dark:text-[#B4B4C8] leading-relaxed">
                  Silakan masukkan kode/token verifikasi yang telah dikirimkan
                  ke email Anda untuk melanjutkan proses pembaruan kata sandi.
                </p>
              </div>
