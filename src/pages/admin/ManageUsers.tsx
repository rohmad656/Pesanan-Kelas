...
export default function ManageUsers() {
  ...
  const handleRoleChange = async (user: any, newRole: string) => {
    ...
    try {
      ...
      toast.success(
        `Peran ${user.name} berhasil diubah menjadi ${newRole.toUpperCase()} (Custom Claims Synced)`,
      );
    } catch (error: any) {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      toast.success(
        "Pengguna berhasil dihapus secara permanen dari Auth & Firestore",
      );
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="bg-white dark:bg-[#27273A] dark:shadow-md border border-slate-200 dark:border-[#3F3F5A]/50 rounded-xl px-4 py-2 flex items-center md:flex-row gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-lg font-extrabold text-slate-900 dark:text-[#F5F5F5] leading-none">
                  {users.length}
                </span>
              </div>
            </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-[#3F3F5A]/30">
                {loading ? (
                    ...
                    >
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm font-medium">
                          Menghubungkan ke Auth & Firestore...
                        </p>
                      </div>
                    </td>
                  </tr>
                  ...
                  currentUsers.map((user) => (
                    <tr
                      key={user.id}
                      className="hover:bg-slate-50/80 dark:hover:bg-white/5 transition-colors border-b border-slate-100 dark:border-[#3F3F5A]/20 last:border-0"
                    >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {user.isOrphan && (
                                <AlertTriangle
                                  ...
                                  aria-label="Data tersimpan, akun Auth dihapus (Status: Nonaktif)"
                                />
                              )}
                            </div>
                            {user.division && (
                      ...
                      <td className="px-6 py-4 text-sm font-medium truncate max-w-[200px] text-slate-600 dark:text-slate-300 align-middle">
                        {user.email}
                      </td>
                        ...
                        {user.isOrphan ? (
                          <div className="flex flex-col">
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              <span className="text-[10px] font-extrabold text-red-600 dark:text-red-400 border border-red-500/20 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                NONAKTIF
                              </span>
                            </div>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 italic">
                              Akun dihapus, data tersisa
                            </span>
                          </div>
                        ) : user.lastLogin ? (
                          <div className="flex flex-col">
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              <span className="text-[10px] font-extrabold text-green-600 dark:text-green-400 border border-green-500/20 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                AKTIF
                              </span>
                            </div>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono">
                              {new Date(
                                user.lastLogin.toMillis?.() || user.lastLogin,
                          ...
                          <div className="flex flex-col">
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              <span className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 border border-slate-500/20 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                NONAKTIF
                              </span>
                            </div>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 italic">
                ...
                {!loading && filteredAndSortedUsers.length === 0 && (
                  <tr>
                    <td
                      colSpan={7}
                      className="px-6 py-12 text-center text-slate-500 dark:text-slate-400 italic font-medium align-middle"
                    >
            ...
            <div className="bg-white dark:bg-[#27273A] rounded-2xl border border-slate-200 dark:border-[#3F3F5A]/30 shadow-sm overflow-hidden">
              <div className="p-4 bg-brand-50 dark:bg-brand-500/5 border-b border-brand-100 dark:border-brand-900/20">
                ...
                <p className="text-[10px] text-slate-500">
                  Kontak yang ditandai aktif akan muncul di sidebar Mahasiswa &
                  Dosen.
                </p>
              </div>
              <div className="divide-y divide-slate-100 dark:divide-[#3F3F5A]/30">
                {contacts.length === 0 ? (
                  ...
                  contacts.map((c) => (
                    <div
                      key={c.id}
                      className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-[#32324A]/50 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={cn(
      ...
      >
        <div className="space-y-6">
          ...
          <p className="text-slate-600 dark:text-[#B4B4C8] text-sm leading-relaxed">
            ...
            ? Tindakan ini akan menghapus data user secara permanen. Data yang
            dihapus tidak dapat dipulihkan lagi.
          </p>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-[#3F3F5A]/20 text-center">
            <p className="text-[10px] text-slate-400 italic">
              Data dihapus dari sistem (Auth & Firestore).
            </p>
          </div>
        </div>
      </BaseModal>
