...
export default function Login() {
  ...
  const getIdentifierPlaceholder = () => {
    if (isForgotPassword && resetStep === "EMAIL")
      return "Masukkan Email atau NIM Anda";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "custom/user-not-found") {
        setError(
          "Akun tidak ditemukan. Pastikan Email atau NIM/NIP sudah benar.",
        );
      } else if (err.code === "auth/invalid-credential") {
        setError(
          "Email belum terdaftar atau kata sandi salah. Silakan daftar dulu atau gunakan login Google.",
        );
      } else if (err.code === "auth/email-already-in-use") {
        setError(
          "Email atau ID ini sudah terdaftar. Silakan masuk menggunakan email tersebut.",
        );
      } else if (err.code === "auth/weak-password") {
        setError("Kata sandi terlalu lemah. Gunakan minimal 6 karakter.");
      } else if (err.code === "auth/operation-not-allowed") {
        setError(
          "Autentikasi Email/Password belum diaktifkan di Firebase Console. Silakan gunakan Login Google untuk saat ini.",
        );
  ...
  const handleGoogleLogin = async () => {
    ...
    const timeoutId = setTimeout(() => {
      if (googleLoading) {
        ...
        setError(
          "Proses login terlalu lama. Silakan coba lagi atau gunakan metode Redirect di bawah ini.",
        );
      }
    ...
    try {
      ...
      if (err.code === "auth/popup-blocked") {
        setShowRedirectOption(true);
        setError(
          "Popup diblokir oleh browser. Silakan izinkan popup untuk situs ini atau gunakan metode Redirect di bawah.",
        );
        ...
        setError(
          `Gagal masuk dengan Google: ${err.message || err.code || "Kesalahan tidak diketahui"}. Tips: Coba buka di Tab Baru.`,
        );
      }
    } finally {
      setGoogleLoading(false);
                  ...
                  >
                    ...
                    <p className="text-slate-500 dark:text-[#B4B4C8] text-sm italic">
                      {isForgotPassword
                        ...
                        : isRegistering
                          ? "Buat akun untuk mengakses layanan kampus"
                    ...
                    <AnimatePresence mode="wait">
                      {isRegistering && (
                        ...
                        >
                          <label
                            htmlFor="name"
                            className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-[#B4B4C8]"
                          >
                            Nama Lengkap
                          </label>
                          <div className="relative group">
                            <User
                              className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-brand-700 dark:text-brand-dark-accent transition-colors"
                              aria-hidden="true"
                            />
                            <input
                              id="name"
                    ...
                    <AnimatePresence mode="wait">
                      {isForgotPassword && resetStep === "CODE" && (
                        ...
                        >
                          ...
                          <p className="text-[10px] text-slate-500 dark:text-[#B4B4C8] mt-2 italic px-1">
                            Tips: Periksa folder <strong>Spam</strong> jika Anda
                            tidak menerima email di Inbox Utama.
                          </p>
                        </motion.div>
                      )}
                        ...
                        >
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-sm text-slate-500 dark:text-[#B4B4C8]">
                              Kata sandi Anda telah diperbarui. Silakan gunakan
                              sandi baru untuk masuk ke akun Anda.
                            </p>
                          </div>
                        </motion.div>
                      )}
                      ...
                      {(!isForgotPassword || resetStep === "EMAIL") && (
                        ...
                        >
                          ...
                          <div className="relative group">
                            <Mail
                              className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 group-focus-within:text-brand-600 dark:text-brand-dark-accent transition-colors"
                              aria-hidden="true"
                            />
                            <input
                              id="nim"
                              ...
                              className={cn(
                                ...
                                validationError
                                  ...
                                  : "border-slate-200 dark:border-[#3F3F5A]/30 focus:border-brand-500 dark:border-brand-dark-accent focus:ring-brand-500/50",
                              )}
                    ...
                    >
                      <div className="relative my-8">
                        <div className="absolute inset-0 flex items-center">
                          <div className="w-full border-t border-slate-100 dark:border-[#3F3F5A]/30"></div>
                        </div>
